...
| 이름 | 역할 | 담당 |
|------|------|------|
| 김규태 | 팀장 | 6축 로봇팔 경로 학습 및 정밀 제어, 쓰러짐 감지, 비상 통신·음성 안내 구현, 시스템 통합 설계 |
| **김수영** | **팀원** | **제스처 인식 설계 및 학습, 로봇팔 제어 연동, WPF 모니터링 UI 설계 및 구현, 얼굴 인증, React 웹 관리자, 안전 시스템 연동** |
| 정서현 | 팀원 | 하드웨어 제어, FSM 설계, 서보모터 도어 제어, 보안실 WPF |
| 강은비 | 팀원 | Flask 서버, 가스·온습도 센서 연동, 다중 비상감지 통합, 서보모터 도어 제어, 환풍기 제어, 비상 이력 관리 |
| 김민준 | 팀원 | 제스처 학습, 시선 추적 커서, 얼굴 캘리브레이션, WPF 통합, 다중 PC 배포 |
